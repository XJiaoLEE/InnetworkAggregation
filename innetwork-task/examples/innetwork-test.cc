...
const std::string linkFilePath = "/home/jenny/workspace/ns-3-allinone/ns-3.37/src/innetwork-task/examples/link.txt";
const std::string aggGropuFilePath = "/home/jenny/workspace/ns-3-allinone/ns-3.37/src/innetwork-task/examples/aggtree.txt";
...
uint16_t stoptime = 50;//5000
std::vector<std::pair<Ipv4Address, std::pair<Ipv4Address, uint16_t>>> fatherAddress;
...
void Createpro (uint16_t port, uint16_t itr, uint8_t rank, std::vector<Address> &sGroup, 
                std::vector<Address> &cGroup, Ptr<Node> node, std::vector<std::pair<Ipv4Address, std::pair<Ipv4Address, uint16_t>>> fatherAddress) {
    Ptr<Producer> producer = CreateObject<Producer> ();
    //cGroup.resize(0);
    cGroup.clear();
    node->AddApplication (producer);
    producer->SetupProducer (port, itr, rank, sGroup, cGroup, basetime * 6, cc,fatherAddress);
...
void Createagg (uint16_t port, uint16_t itr, uint8_t rank, std::vector<Address> &sGroup, 
    ...
    aggragator->SetupAggregator (port, itr, rank, sGroup, cGroup, basetime, cc,fatherAddress);
...
void Createcon (uint16_t port, uint16_t itr, uint8_t rank, uint16_t vsize,
                    ...
                    Ptr<Node> node, std::vector<std::pair<Ipv4Address, std::pair<Ipv4Address, uint16_t>>> fatherAddress) {
    Ptr<Consumer> consumer = CreateObject<Consumer> ();
    node->AddApplication (consumer);
    consumer->SetupConsumer (port, itr, rank, vsize, sGroup, cGroup, basetime, cc,fatherAddress);
...
void CreateAggTree (std::string &nodeName, std::vector<Address> pa,
    ...
    if (nodeName. find ("con") != std::string::npos)
        Createcon(server_port, itr, rank, vsize, pa, cGroup, Names::Find<Node> (nodeName),fatherAddress);
    if (nodeName. find ("agg") != std::string::npos)
        Createagg(server_port, 0, rank, pa, cGroup, Names::Find<Node> (nodeName),fatherAddress);
    if (nodeName. find ("pro") != std::string::npos)
        Createpro(server_port, 0, rank, pa, cGroup, Names::Find<Node> (nodeName),fatherAddress);
}
...
void CreateAggTreeTopo (uint16_t itr, uint16_t vsize, uint16_t server_port) {
    ...
    //std::cout << aggGroups.size() <<std::endl;
}
...
void CreateDirectTopo (NodeContainer &cons, NodeContainer &pros, uint16_t itr, 
    ...
    for (uint8_t i = 0; i < pros. GetN (); ++i) {
        ...
        producer->SetupProducer (server_port, 0, rank, consumers, nullV, basetime * 2, cc,fatherAddress);
        rank++;
        producer->SetStartTime (Seconds(starttime));
        producer->SetStopTime (Seconds(stoptime));
    }
    ...
    consumer->SetupConsumer (server_port, itr, rank, vsize, nullV, producers, basetime, cc,fatherAddress);
